...
import { Heart, Bell, BellOff, MessageCircle, ShoppingCart, Trash2, ArrowLeft } from "lucide-react";
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-gradient-to-br from-[#F4F2FF] via-[#F8F7FF] to-[#F0F5FF]">
      ...
      <div className="flex min-w-0 flex-1 flex-col h-full">
        <main className="flex-1 overflow-y-auto p-7 animate-fade-in-up">
          ...
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-2xl font-bold text-[#17152A]">Wishlist</h1>
            <p className="text-sm text-gray-500">Track listings you've saved and configure price drop notifications.</p>
          </div>
          ...
          {wishlist.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {wishlist.map((item) => (
                <div key={item.id} className="rounded-2xl bg-white border border-gray-100 p-5 shadow-sm flex flex-col justify-between hover:shadow-md transition">
                  
                  {/* Top content */}
                  <div>
                    <div className="flex justify-between items-start">
                      <div className={`h-24 w-16.5 rounded-lg bg-gradient-to-br ${item.coverClass} flex items-center justify-center text-[9px] font-extrabold text-white uppercase tracking-wider border border-black/5`}>
                      ...
                      <div className="flex gap-1.5">
                        <button
                          ...
                          className={`p-2 rounded-xl border transition cursor-pointer ${
                            item.alertActive
                              ? "bg-amber-50 border-amber-100 text-amber-500 hover:text-amber-600"
                              : "bg-white border-gray-200 text-gray-400 hover:text-gray-600"
                          }`}
                          title={item.alertActive ? "Price drop alerts active" : "Enable price drop alerts"}
                        >
                          {item.alertActive ? <Bell size={14} /> : <BellOff size={14} />}
                        </button>
                        <button
                          onClick={() => handleRemove(item.id)}
                          className="p-2 rounded-xl border border-red-100 bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-700 transition cursor-pointer"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
                    ...
                    <div className="mt-4">
                      ...
                      <h3 className="font-bold text-[#17152A] text-sm mt-2 line-clamp-1">{item.title}</h3>
                      <p className="text-xs text-gray-400 mt-0.5">{item.author}</p>
                      <p className="text-sm font-extrabold text-[#6C4BF4] mt-2">{item.price}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 pt-4 border-t border-gray-100 flex gap-2">
                    <button
                      onClick={() => alert(`Redirecting to checkout for ${item.title}`)}
